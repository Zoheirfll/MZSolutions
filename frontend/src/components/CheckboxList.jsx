...
export default function CheckboxList({ items, selected, onToggle, emptyLabel }) {
  ...
  return (
    <div className="max-h-40 overflow-y-auto rounded-lg border divide-y" style={{ borderColor: theme.dark.border }}>
      {items.map(item => (
        <label key={item.id} className="flex items-center gap-2.5 px-3 py-2 text-sm text-app-primary cursor-pointer hover:bg-violet-500/5 transition" style={{ borderColor: theme.dark.border }}>
