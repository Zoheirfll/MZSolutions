  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ background: 'rgba(0,0,0,0.7)' }} onClick={onClose}>
      <div className="w-full max-w-md rounded-xl border p-6" style={{ background: theme.dark.card, borderColor: theme.dark.border }} onClick={e => e.stopPropagation()}>
        <h3 className="font-semibold text-app-primary mb-5">{isEdit ? 'Modifier' : 'Ajouter'} {TABS.find(t => t.value === pixelType)?.label}</h3>
        <form onSubmit={submit} className="space-y-4">
          <div>
            <label className="block text-xs text-app-muted-light mb-1.5">Identifiant *</label>
          ...
          <div>
            <label className="block text-xs text-app-muted-light mb-1.5">Nom (optionnel)</label>
      ...
      <div className="flex items-center gap-2 mb-6 flex-wrap">
        {TABS.map(t => (
          <button key={t.value} onClick={() => setTab(t.value)}
            className={`px-4 py-1.5 rounded-lg text-sm font-medium transition flex items-center gap-1.5 ${tab === t.value ? 'text-white bg-violet-600' : 'text-app-muted-light hover:text-app-primary hover:bg-violet-500/5'}`}
            ...
            <table className="w-full text-sm min-w-160">
              ...
              <tbody>
                {currentPixels.length === 0 ? (
                  <tr><td colSpan={5} className="px-4 py-10 text-center text-sm text-app-muted">Aucun pixel configuré pour l'instant.</td></tr>
                ) : currentPixels.map(p => (
                  <tr key={p.id} className="border-b hover:bg-violet-500/5 transition" style={{ borderColor: theme.dark.borderRowHover }}>
                    <td className="px-4 py-3 text-app-primary">{p.label || '—'}</td>
                    <td className="px-4 py-3 text-app-primary font-mono text-xs">{p.pixel_id}</td>
                    ...
                    <td className="px-4 py-3 text-app-muted text-xs">{new Date(p.created_at).toLocaleDateString('fr-DZ')}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <button onClick={() => setEditingPixel(p)} className="text-xs text-app-primary hover:bg-violet-500/10 px-2 py-1 rounded transition cursor-pointer">Modifier</button>
