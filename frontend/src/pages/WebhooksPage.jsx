            ...
            <div className="flex flex-wrap gap-2">
              {catalog.map(ev => (
                <button key={ev.key} type="button" onClick={() => toggleEvent(ev.key)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${events.includes(ev.key) ? 'text-white bg-violet-600' : 'text-app-muted-light hover:text-app-primary'}`}
