...
### Transporteurs — 29 sociétés de livraison (17 réels via Ecotrack, reste mocké)
...
- **Découverte clé — plateforme partagée Ecotrack** : plusieurs transporteurs tournent sur la **même plateforme technique** (ecotrack.dz), repérée via le projet open-source [PiteurStudio/CourierDZ](https://github.com/PiteurStudio/CourierDZ) (implémentation PHP de référence). `backend/orders/carriers/ecotrack.py::EcotrackClient` implémente **une seule fois** l'appel réel (`POST {sous-domaine}api/v1/create/order`, header `Authorization: Bearer {token}`) — chaque fichier transporteur hérite juste de cette classe en changeant `api_domain`. Retombe sur `MockCarrierClient` si `api_token` vide (aucun compte configuré). `backend/orders/wilaya_codes.py` fait la conversion nom de wilaya (`Order.wilaya`) → code numérique (1-58) attendu par l'API, miroir de `frontend/src/data/wilayas.js`
  ...
  - **Noest** : PAS un partenaire Ecotrack malgré les apparences (son propre nom "ECO TRACK" trouvé dans une recherche est un terme générique, pas ecotrack.dz) — leur plateforme pro (`app.noest-dz.com`) répond en 404 sur les endpoints standards Ecotrack, signe d'une API interne différente/propriétaire
  - **12 restants mockés**, sous-domaine non trouvé par essais successifs (`MockCarrierClient`, tracking factice `MOCK-{carrier}-{order_id}-{uuid6}`) ou API propre confirmée différente : `guepex`, `waslet`, `ecomdz`, `overed`, `zimou_express`, `mdm`, `noest`, `yalidine`, `zr_express`, `speedmail`, `yalitec`, `maystro`. Pour en brancher un des non-confirmés : tester une requête réelle sur `https://{sous-domaine-supposé}.ecotrack.dz/api/v1/get/wilayas` (401 = existe, 404/timeout = à écarter)
  - ⚠️ **Non testé en conditions réelles** — seule la couche connexion/auth est confirmée (401 sans token = serveur actif) ; **aucune création d'expédition réelle n'a été testée** faute d'un vrai token pour l'un de ces 17 (aucun compte transporteur ouvert à ce stade). Le format de requête (`nom_client`, `telephone`, `adresse`, `code_wilaya`...) vient d'une lib open-source vérifiée en production, donc probablement correct, mais pas garanti à 100% pour chaque revendeur en marque blanche. **`CarrierAccount.departure_wilaya`** (ville de départ saisie par le vendeur dans l'UI) **n'est actuellement pas envoyé** dans le payload — le schéma Ecotrack ne décrit que la destination, le point de départ semble géré côté configuration du compte Ecotrack lui-même, pas par requête, mais ceci reste à confirmer avec un vrai compte
