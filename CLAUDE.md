...
Testé via `manage.py test ai_assistant` (22 tests : dégradation propre sur panne simulée pour les deux fournisseurs, gating de permission sur chaque endpoint et chaque tool, tool calling multi-tours) + vérification en conditions réelles avec une vraie clé Groq (cycle complet chat + appel d'outil + réponse finale) + suite frontend complète (377 tests, aucune régression).
