...
Client HTTP unique `ai_assistant/ollama_client.py` (`chat()`/`generate()`) — `OLLAMA_BASE_URL`/`OLLAMA_MODEL` en variables d'environnement (jamais figés en dur, défaut `http://localhost:11434`/`llama3.1:8b`). Toute panne Ollama (timeout/connexion refusée) → `503` explicite, jamais un 500, jamais un blocage du flux appelant (best-effort, même philosophie que `webhooks.dispatch.fire_event`).
...
**Déploiement** : service `ollama` dans `docker-compose.yml` (volume `ollama_data`, `backend` en dépend). Le modèle n'est pas tiré au build — après le premier démarrage prod : `docker compose exec ollama ollama pull llama3.1:8b`. En dev local, installer Ollama nativement (`OLLAMA_BASE_URL=http://localhost:11434`, valeur par défaut, rien à configurer).

Testé via `manage.py test ai_assistant` (18 tests : dégradation propre sur panne Ollama simulée, gating de permission sur chaque endpoint et chaque tool, tool calling multi-tours) + suite frontend complète (377 tests, aucune régression).
