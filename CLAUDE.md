...
- **Yalidine en pratique** : inscription pro sur `app.yalidine.ae/register` puis Réglages → API → Generate pour l'ID/Token (nécessite un dossier professionnel — RC ou carte d'artisan envoyé par email à contact@yalidine.com — pas un simple formulaire self-service). Auth différente d'Ecotrack (`token` + `key`, pas Bearer seul). Supportent aussi les webhooks (Développement → Gérer les webhooks) pour les mises à jour de statut en temps réel, même principe que Shopify
